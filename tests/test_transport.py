...
async def test_custom_serializers(auto_close):
    serializer = object()
...
async def test_no_sniff_on_start(auto_close):
    t = auto_close(AIOHttpTransport([{}], sniff_on_start=False))
    assert t.initial_sniff_task is None


async def test_sniff_on_start(auto_close, es_server):
...
async def test_close_with_sniff_on_start(es_server):
...
async def test_get_connection_with_sniff_on_start(auto_close, es_server):
...
async def test_get_connection_with_sniffer_timeout(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': 'unknown_host',
                                      'port': 9200},
                                     {'host': es_server['host'],
                                      'port': es_server['port']}],
...
async def test_get_connection_without_sniffer_timeout(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': 'unknown_host',
                                      'port': 9200},
                                     {'host': es_server['host'],
                                      'port': es_server['port']}],
                                    http_auth=es_server['auth'],
                                    sniffer_timeout=1e12))
    conn = await t.get_connection()
    assert conn is not None
    assert t.initial_sniff_task is None
...
async def test_sniff_hosts_error(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': 'unknown_host',
                                      'port': 9200}]))
...
async def test_sniff_hosts_no_hosts(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': es_server['host'],
                                      'port': es_server['port']}],
                                    http_auth=es_server['auth']))
...
async def test_mark_dead(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': 'unknown_host',
                                      'port': 9200},
                                     {'host': es_server['host'],
                                      'port': es_server['port']}],
...
async def test_mark_dead_with_sniff(auto_close, es_server):
    t = auto_close(AIOHttpTransport([{'host': 'unknown_host',
                                      'port': 9200},
                                     {'host': 'unknown_host2',
                                      'port': 9200},
...
async def test_send_get_body_as_post(es_server, auto_close):
...
async def test_send_get_body_as_source(es_server, auto_close):
    cl = auto_close(Elasticsearch([{'host': es_server['host'],
                                   'port': es_server['port']}],
...
async def test_send_get_body_as_get(es_server, auto_close):
    cl = auto_close(Elasticsearch([{'host': es_server['host'],
                                   'port': es_server['port']}],
                                  http_auth=es_server['auth']))
    await cl.create('test', '1', {'val': '1'})
...
async def test_send_get_body_as_source_none_params(es_server, auto_close):
    cl = auto_close(Elasticsearch([{'host': es_server['host'],
                                   'port': es_server['port']}],
                                  send_get_body_as='source',
                                  http_auth=es_server['auth']))
...
async def test_request_error_404_on_head(auto_close):
    exc = TransportError(404)
    t = AIOHttpTransport([{}], connection_class=DummyConnection, exception=exc)
    auto_close(t)
...
async def test_request_connection_error(auto_close):
    exc = ConnectionError()
    t = AIOHttpTransport([{}], connection_class=DummyConnection, exception=exc)
    auto_close(t)
...
async def test_request_connection_timeout(auto_close):
...
async def test_request_connection_timeout_with_retry(auto_close):
    exc = ConnectionTimeout()
    t = AIOHttpTransport([{}], connection_class=DummyConnection, exception=exc,
...
async def test_request_retry_on_status(auto_close):
...
async def test_request_without_data(auto_close):
    t = AIOHttpTransport([{}], connection_class=DummyConnection, data='')
    auto_close(t)
...
async def test_request_headers(auto_close, es_server, mocker):
    t = auto_close(AIOHttpTransport(
        [{'host': es_server['host'],
          'port': es_server['port']}],
        http_auth=es_server['auth'],
